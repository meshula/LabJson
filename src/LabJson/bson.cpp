...
int bson_append_element( bson *b, const char *name_or_null, const bson_iterator *elem ) {
    ...
    if ( name_or_null == NULL ) {
        ...
        int data_size = size - 2 - (int) strlen( bson_iterator_key( elem ) );
...
int bson_append_start_object( bson *b, const char *name ) {
    ...
    b->stack[ b->stackPos++ ] = (int)(b->cur - b->data);
...
/* dp: mongo must define this somewhere else - I removed the extern and the const */
...
void bson_numstr( char *str, int i ) {
    ...
    if (initStrs) {
        ...
        for (int n = 0; n < 1000; ++n)
            bson_sprintf( (char*) bson_numstrs[n], "%d", n );
    }
    /* dp: end */
